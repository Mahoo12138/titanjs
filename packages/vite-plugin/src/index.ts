...
export function titanVitePlugin(options: TitanVitePluginOptions): Plugin {
  const { rootDir, config, devSession, onFileChange } = options
  ...
  return {
    ...
    configureServer(viteServer: ViteDevServer) {
      ...
      viteServer.watcher.on('change', async (filePath) => {
        if (!filePath.startsWith(sourceDir) || !filePath.endsWith('.md')) return
        ...
        if (devSession) {
          try {
            ...
            viteServer.ws.send({
              ...
              event: 'titan:routes-updated',
              data: {
                affectedRoutes: devSession.getRoutes().map(route => route.url),
                entryId: null,
      ...
      return () => {
        viteServer.middlewares.use(async (req, res, next) => {
          ...
          try { urlPath = decodeURIComponent(urlPath) } catch { /* malformed */ }
    ...
    import.meta.hot.on('titan:routes-updated', (data) => {
      ...
      if (data.affectedRoutes.includes(normalized) || data.affectedRoutes.includes(currentPath)) {
        window.location.reload()
            ...
            const injectedHtml = html.replace('</head>', hmrScript + '\n</head>')

            // Transform through Vite's pipeline (for HMR client injection)
            const transformed = await viteServer.transformIndexHtml(urlPath, injectedHtml)
